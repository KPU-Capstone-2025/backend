...
class ChatServiceImpl(
    ...
    override fun askQuestion(monitoringId: String, userQuestion: String): String {
        ...
        val alertContext = if (recentAlerts.isEmpty()) {
            ...
            recentAlerts.joinToString("\n") {
                "[${it.severity}] ${it.alertName} (${it.createdAt.format(DateTimeFormatter.ofPattern("MM-dd HH:mm"))}): ${it.description}"
            }
        }
        ...
        val logContext = if (recentLogs.isEmpty()) {
            ...
            val errorPart = if (errorLogs.isNotEmpty())
                "=== ERROR/WARN 로그 (${errorLogs.size}건) ===\n" + errorLogs.take(20).joinToString("\n") { "[${it.severity}] ${it.body}" }
            else "ERROR/WARN 로그 없음"
    ...
    override fun getChatHistory(monitoringId: String): List<ChatHistoryResponse> {
