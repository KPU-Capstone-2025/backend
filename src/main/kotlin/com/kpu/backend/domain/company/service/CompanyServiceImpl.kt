...
class CompanyServiceImpl(
    ...
    override fun registerAndProvision(req: CompanyRegisterRequest) {
        ...
        try {
            ...
            saveCompany(req, monitoringId, monitoringIp = privateIp)
        } catch (e: Exception) {
            log.error("인프라 구성 실패: {}", e.message, e)
            throw BusinessException(ErrorCode.INTERNAL_SERVER_ERROR, "인프라 구성 실패: ${e.message}")
        }
    }
