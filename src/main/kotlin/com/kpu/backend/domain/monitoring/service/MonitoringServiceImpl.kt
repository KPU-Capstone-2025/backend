...
class MonitoringServiceImpl(
    ...
    override fun getContainerList(companyId: Long, hostName: String?): List<ContainerStatus> {
        ...
        return queryPrometheus(query, prometheusUrl(ip)).mapNotNull {
            ...
            if (!name.isNullOrBlank() && name != "metric-agent") ContainerStatus(containerId = name, status = "RUNNING")
            else null
        }.distinctBy { it.containerId }
    }

    override fun getHostMetrics(companyId: Long, hostName: String?): ResourceMetrics {
    ...
    override fun getHostMetricsHistory(companyId: Long, rangeMinutes: Int, step: Int, hostName: String?): List<Map<String, Any>> {
        val company = companyRepository.findById(companyId).orElse(null) ?: return emptyList()
        val ip = company.ip ?: return emptyList()
        val promUrl = prometheusUrl(ip)
    ...
    override fun getDiscoveredHosts(companyId: Long): List<String> {
        val company = companyRepository.findById(companyId).orElse(null) ?: return emptyList()
        val ip = company.ip ?: return emptyList()
    ...
    override fun getContainerMetrics(companyId: Long, containerName: String): ResourceMetrics {
        val company = companyRepository.findById(companyId).orElse(null)
            ?: return ResourceMetrics(status = "NOT_FOUND", cpuUsage = 0.0, memoryUsage = 0.0, diskUsage = 0.0, networkTraffic = 0.0)
        val ip = company.ip
            ?: return ResourceMetrics(status = "NOT_FOUND", cpuUsage = 0.0, memoryUsage = 0.0, diskUsage = 0.0, networkTraffic = 0.0)
    ...
    override fun getLogs(companyId: Long, containerName: String?, severity: String?, keyword: String?, limit: Int, hostName: String?): List<LogEntry> {
        val company = companyRepository.findById(companyId).orElse(null) ?: return emptyList()
        val ip = company.ip ?: return emptyList()
                ...
                for (v in values) {
                    ...
                    try {
                        ...
                        detectedSev = when {
                            ...
                            body.contains("warn", true) || body.contains("warning", true) || body.contains("slow", true) -> "WARN"
                            else -> originalSev
                        }
                    } catch (e: Exception) {
    ...
    override fun getMonthlyMetrics(companyId: Long, year: Int, month: Int?, startDate: String?, endDate: String?, hostName: String?): MonthlyMetricsResponse {
        val company = companyRepository.findById(companyId).orElse(null)
    ...
    override fun getAlertsByDate(companyId: Long, date: String, hostName: String?): List<AlertLog> {
        val company = companyRepository.findById(companyId).orElse(null) ?: return emptyList()
    ...
    override fun getLogsByDateRange(companyId: Long, date: String): List<LogEntry> {
        val company = companyRepository.findById(companyId).orElse(null) ?: return emptyList()
        val ip = company.ip ?: return emptyList()
    ...
    override fun getUserUsage(companyId: Long, hostName: String?): List<UserUsageStat> {
        val company = companyRepository.findById(companyId).orElse(null) ?: return emptyList()
        val ip = company.ip ?: return emptyList()
        val promUrl = prometheusUrl(ip)
        val hf = if (!hostName.isNullOrBlank()) "{host_name=\"$hostName\"}" else ""
